<template>
  <div :style="{'height':getTabHeight}">
    ...
    <v-row>
      <v-col cols='12'>
        <div ref="musicLyric" class="lyric" :class="$vuetify.breakpoint.smAndDown?'smHeight':'lgHeight'">

          <Scroll :data='lyric.length' ref="musicScroll">
            <div class="lyric-items body-2">
              <template v-if="lyric.length">
                <p :class='{activeLyric:lyricIndex===index}' v-for='(item,index) in lyric' :key='index' ref="lyrics">
                  {{ item.text }} </p>
              </template>
